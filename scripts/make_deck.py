...
def build() -> Path:
    ...
    _table(s, Inches(0.62), Inches(1.55), Inches(12.1), [
        ...
        ["규칙만 사용 (LLM 없이)", "문서유형은 제목으로 가르지만, 비정형 서류에서 필드를 읽지 못함",
         "실측(규칙 단독, 실물 15건) — 유형 판정 15/15, 필드는 자리 165개 중 63개(38%)만 채움"],
        ["채택: LLM 판독 + 규칙 판정 + 원문 대조",
         "읽기는 LLM이 잘하고, 판정은 재현 가능해야 한다 — 역할을 분리",
         "실측(전수 감사, 실물 25건) — 이상 0건"],
    ...
    _table(s, Inches(0.62), Inches(3.5), Inches(12.1), [
        ...
        ["점검에 쓰지 않은 자료", "감사에 쓰지 않은 실물 23건(KB·삼성·미래에셋·신한·대우)으로 재검증",
    ...
    _card(s, Inches(9.15), Inches(1.5), Inches(3.57), Inches(4.4),
          ...
          ["규칙마다 근거 조문을 확정 선언 — 검색어가 바뀌어도 근거가 흔들리지 않는다",
           ...
           "판정 근거는 문서·필드 단위로 남기고, '원문 근거 보기'로 그 값이",
           "   실제로 있는 PDF 위치를 좌표째 펼쳐 준다",
           "적용 조항과 근거가 내보내기 파일에도 같이 담긴다"], accent=KB_YELLOW)
    ...
    _table(s, Inches(0.62), Inches(4.5), Inches(12.1), [
        ...
        ["검사 규칙 8종 + 법령 조문 매핑", "완료 · 회귀 테스트 97건"],
