...
def check_recording_requirement(
    ...
    return RuleCheck(
        ...
        # 위험등급만으로 녹취 의무를 확정할 수는 없다(상품 종류·내부 기준을 함께 본다).
        # 그 사실을 밝혀야 '2등급이면 무조건 녹취냐'는 오해가 생기지 않는다.
        # 앞 문장은 그대로 둔다 — 음성 미판독 고지는 회귀 테스트가 지키는 약속이다.
        suggestion="이 도구는 음성 파일을 판독하지 않습니다. 서류에서 읽히는 위험등급으로 "
        "대상 후보만 짚으므로, 실제 녹취 의무 해당 여부는 상품 종류·내부 기준과 함께 "
        "확인하고, 해당한다면 녹취 기록이 보관돼 있는지 확인하세요.",
    )
