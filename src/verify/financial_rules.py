...
# '여부'(원금보장 여부)는 중립적 질의 표현이지 보장 약속이 아니다.
# 예전에는 '여' 한 글자만으로 예외 처리했다 — 쪽번호가 낱말 사이에 끼어 '여부'가
# 쪼개지는 실물 사례 때문이었다. 그런데 그 쪽번호는 _PAGE_ARTIFACT 가 매칭 전에
# 이미 지우므로 '여부'는 온전히 남는다(재구성해 확인).
# 반면 '여' 한 글자는 '하여·위하여·관하여'를 전부 삼켜서, "수익을 보장하여
# 드립니다" 같은 명백한 위반을 놓쳤다(실측). 그래서 '여부'로 좁힌다.
_CLAIM_EXCEPTIONS = (
    "되지", "되지않", "않", "아닙", "아니", "없는", "불가", "추구", "형", "여부", "제외",
)
